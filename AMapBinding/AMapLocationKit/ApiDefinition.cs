...
namespace AMapLocationKit
{
	...
	interface AMapLocationPolygonRegion
	{
		...
		unsafe IntPtr Constructor(CLLocationCoordinate2D coordinates, nint count, string identifier);
	}
	...
	interface AMapLocationManager
	{
		...
		// @property (assign, nonatomic) AMapLocationAccuracyMode locationAccuracyMode __attribute__((availability(ios, introduced=14.0)));
		[iOS(14, 0)]
		[Export("locationAccuracyMode", ArgumentSemantic.Assign)]
		AMapLocationAccuracyMode LocationAccuracyMode { get; set; }

		//TODO
		// @property (readonly, nonatomic) CLAccuracyAuthorization currentAuthorization __attribute__((availability(ios, introduced=14.0)));
		//[iOS(14, 0)]
		//[Export("currentAuthorization")]
		//CLAccuracyAuthorization CurrentAuthorization { get; }
	...
	{
		// @optional -(void)amapLocationManager:(AMapLocationManager *)manager doRequireTemporaryFullAccuracyAuth:(CLLocationManager *)locationManager completion:(void (^)(NSError *))completion;
		[Export("amapLocationManager:doRequireTemporaryFullAccuracyAuth:completion:")]
		void AmapLocationManager(AMapLocationManager manager, CLLocationManager locationManager, Action<NSError> completion);
		...
		// @optional -(void)amapLocationManager:(AMapLocationManager *)manager locationManagerDidChangeAuthorization:(CLLocationManager *)locationManager;
		[Export("amapLocationManager:locationManagerDidChangeAuthorization:")]
		void AmapLocationManagerLocationManagerDidChangeAuthorization(AMapLocationManager manager, CLLocationManager locationManager);
	...
	interface AMapGeoFenceManager
	{
		...
		unsafe void AddPolygonRegionForMonitoringWithCoordinates(CLLocationCoordinate2D coordinates, nint count, string customID);
		...
		void AddKeywordPOIRegionForMonitoringWithKeyword(string keyword, string type, string city, nint size, string customID);
		...
		void AddAroundPOIRegionForMonitoringWithLocationPoint(CLLocationCoordinate2D locationPoint, nint aroundRadius, string keyword, string type, nint size, string customID);
	...
	{
		// @required -(void)amapLocationManager:(AMapGeoFenceManager *)manager doRequireTemporaryFullAccuracyAuth:(CLLocationManager *)locationManager completion:(void (^)(NSError *))completion;
		[Abstract]
		[Export("amapLocationManager:doRequireTemporaryFullAccuracyAuth:completion:")]
		void AmapLocationManager(AMapGeoFenceManager manager, CLLocationManager locationManager, Action<NSError> completion);
		...
		void AmapGeoFenceManager(AMapGeoFenceManager manager, CLLocationManager locationManager);
		...
		void AmapGeoFenceManager(AMapGeoFenceManager manager, AMapGeoFenceRegion[] regions, string customID, NSError error);
		...
		void AmapGeoFenceManager(AMapGeoFenceManager manager, AMapGeoFenceRegion region, string customID, NSError error);
	}
}
